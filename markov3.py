...
# ###############
    ...
    # loop over text_list by start_index this many times: leng of text_list - n ish
    #     find a slice of n words from start_index : start_index + n
    #         turn slice into a tuple called ngram
    #         next word = text_list[start_index+n]
    #         if ngram is in chains.keys()
    #             append next word to value associated with ngram
    #         else
    #             key = ngram
    #             value = []
    #             create new item in dictionary with key and append next word to value

    start_index = 0

    for start_index in range(len(text_list)-n):
        ngram = text_list[start_index:start_index+n]
        ngram = tuple(ngram)
        next_word = text_list[start_index+n]
